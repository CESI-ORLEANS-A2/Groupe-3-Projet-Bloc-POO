...
Request^ ProductRequestMapping::addProduct(String^name, String^description, float cost) {
    try {
        MySqlCommand^ cmd = gcnew MySqlCommand();
        cmd->CommandText = "INSERT INTO Product VALUES(name,description,cost)";
        cmd->Prepare();
...
Request^ ProductRequestMapping::updateProduct(int id,String^ name, String^ description, float cost) {
    try {
        MySqlCommand^ cmd = gcnew MySqlCommand();
        cmd->CommandText = "UPDATE table SET name = @name, description = @description, cost = @cost WHERE id = @id;";
        cmd->Prepare();
        cmd->Parameters->AddWithValue("@name", name);
        cmd->Parameters->AddWithValue("@description", description);
        cmd->Parameters->AddWithValue("@cost", cost);
